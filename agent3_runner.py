...
def execute_trade_on_gains(signal):
    ...
    try:
        ...
        try:
            ...
            available_functions = dir(contract.functions)
            print(f"Available contract functions: {[f for f in available_functions if not f.startswith('__')]}")
            
            # Create a named tuple that matches the struct format
            tuple_data = (
            ...
            try:
                ...
                txn = contract.functions.openTrade(
                    tuple_data,       # The trade struct tuple
                    slippage,         # Slippage tolerance in 0.1%
                ...
                print("Successfully built transaction using tuple approach")
            except Exception as e:
                print(f"First approach failed: {str(e)}")
                
                # Try alternative approach with dictionary
                trade_dict = {
                ...
                print(f"Trying fallback approach with trade_dict")
                txn = contract.functions.openTrade(
                    trade_dict,
                    slippage,
                    account.address
                ...
                print("Successfully built transaction using dictionary approach")
            
            # Add a generous gas limit
            txn['gas'] = 500000  # Using a higher gas limit to ensure transaction goes through
